...
db-start: ## start the database server
	...
	docker run --rm --name postgres -v $(shell pwd)/testdata:/testdata \
		...
		-e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=melon_service -d -p 5432:5432 postgres
...
.PHONY: bazel-build
bazel-build: ## update dependency and build using bazel
	bazel run //:gazelle -- update-repos -from_file=go.mod -to_macro=deps.bzl%go_dependencies
	bazel build //...

.PHONY: bazel-test
bazel-test: ## run all test with bazel
	bazel test //...

.PHONY: bazel-clean
bazel-clean: ## use bazel clean to remove all bazel output folders
	bazel clean
